...
function generateStudentId(admissionYear, classId) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log(`Generating Student ID for Class ID: ${classId}`);
        try {
            // Step 1: Fetch the class data by ID
            const classData = yield class_model_1.Class.findById(classId).lean();
            if (!classData) {
                throw new Error("Class not found.");
            }
            ...
            let studentExists = yield student_model_1.Student.exists({ studentId: newStudentId });
            let attempts = 0;
            // If studentId exists, increment the number and try again
            while (studentExists && attempts < 10) {
                lastThreeDigits++;
                ...
                studentExists = yield student_model_1.Student.exists({ studentId: newStudentId });
                attempts++;
            }
            if (studentExists) {
                throw new Error("Unable to generate a unique student ID after several attempts.");
